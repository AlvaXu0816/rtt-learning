...
__again:
        ...
        else
        {
            ...
            else
            {
                ...
                if (thread->current_priority < mutex->owner->current_priority)  //B优先级<A优先级 B要提升A的优先级
                {
                ...
                rt_ipc_list_suspend(&(mutex->parent.suspend_thread),    //B休眠：ReadyList移除 //B记录下来
                                    thread,
...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
    ...
    if (thread != mutex->owner)     //判断是否是线程拥有者
    {
        thread->error = -RT_ERROR;
    ...
    mutex->hold --;
    /* if no hold */
    if (mutex->hold == 0)   //如果持有计数减少到 0，说明当前线程已完全释放了互斥锁
    {
